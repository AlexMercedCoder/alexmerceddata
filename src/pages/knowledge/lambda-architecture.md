...
Once or twice a day, a massive batch processing engine (like Apache Spark) wakes up. It reads the *entire* historical dataset from the beginning of time and recalculates all the analytical views. 
...
To solve the latency problem, the exact same raw data is simultaneously routed into a real-time streaming engine (like Apache Flink or Kafka Streams).
...
Business analysts do not want to query two different systems. The Serving Layer is a database (often a NoSQL database like Cassandra or an OLAP engine like Apache Druid) designed to merge the results.
