...
*   **[Data Lakehouse](/knowledge/data-lakehouse) Queries**: An agent can be given a `run_sql` tool. When the CEO asks for a revenue report, the Agent automatically writes the SQL, uses the tool to query the [Dremio](/knowledge/dremio) Lakehouse, and returns the chart.
