...
*   **Batch Ingestion**: The engineer writes a script (using tools like Airbyte or Fivetran) that wakes up at 2:00 AM, connects to the Salesforce API, downloads the last 24 hours of data, and dumps it into [Amazon S3](/knowledge/amazon-s3).
*   **[Streaming Ingestion](/knowledge/streaming-ingestion)**: The engineer sets up [Apache Kafka](/knowledge/apache-kafka). Every time a user clicks a button on the website, a JSON message is instantly fired into Kafka, which streams it continuously into the [Data Lakehouse](/knowledge/data-lakehouse) in real-time.

## Phase 3: Storage

The ingested data needs a home. 
*   **The Data Lake**: The raw, untouched data (JSON, CSV, Parquet) lands in Cloud Object Storage (S3, ADLS, GCS). This is the "Bronze" layer of the [Medallion Architecture](/knowledge/medallion-architecture).
*   **The Architecture**: The engineer must choose the correct storage formats ([Apache Parquet](/knowledge/apache-parquet)) and the correct table architectures ([Apache Iceberg](/knowledge/apache-iceberg)) to ensure the data is mathematically optimized for future analytical queries.
...
*   **The Workhorse**: This is where the majority of Data Engineering happens. Using tools like [Apache Spark](/knowledge/apache-spark) or **dbt (data build tool)**, the engineer cleans, filters, and mathematically aggregates the data.
...
*   **Business Intelligence (BI)**: The engineer connects the Gold tables to a dashboarding tool like [Tableau](/knowledge/tableau), PowerBI, or Superset, so the CEO can view the daily revenue charts.
*   **Machine Learning**: The engineer serves the clean data to the Data Science team to train a predictive AI model.
*   **[Reverse ETL](/knowledge/reverse-etl)**: The engineer pushes the calculated "Lifetime Customer Value" metric *out* of the Data Lakehouse and back *into* the operational Salesforce database, so the sales team can see it directly in their CRM.
